...
public class StockBackView extends JDialog{
	...
	public StockBackView(Frame f, String title, boolean bool){
		super(f,title,bool);
		uService = new UserBaseServiceImpl();
		service = new FirmManageServiseImpl();
		mScervice= new ModleInfoServiceImpl();
	...
	private void init() {
		...
		txtNub.setText(StockView.billID+"");
		...
		btnPartsAdd.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				new BtnPartsView(new Frame(), "配件导入", true,bv);
	...
	public void setTable(List<Parts> list,int num) {
		...
		if(list != null){
			...
			for(Parts f:list){
				...
				List<Modle> modle = mScervice.modleExportById(f.getpModle());
				Modle m = modle.get(0);
